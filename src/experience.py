...
class Experience:
	...
	def get_state(self, obs):
		...
		self.return_state[:-1, :, :] = self.obs.take(
					np.arange(self.top - self.frames_per_state + 1, self.top)
					, axis = 0, mode = 'wrap')
	...
	def get_random_minibatch(self, mbsize):
		...
		cnt = 0
		while cnt < mbsize:
			start_id = self.rng.randint(1, self.len - self.frames_per_state + 1
				, mbsize - cnt) + self.top * (self.len >= self.replay_mem_size)
			end_id = start_id + self.frames_per_state - 1
			not_terminal = [not np.any(self.terminal.take(
							np.arange(i - 1, j - 1), axis = 0, mode = 'wrap')) 
			...
			action[cnt : cnt + num_ok] = self.action.take(
				end_id[not_terminal] - 1, mode = 'wrap').reshape(-1, 1)
			reward[cnt : cnt + num_ok] = self.reward.take(
				end_id[not_terminal] - 1, mode = 'wrap').reshape(-1, 1)
			terminal[cnt : cnt + num_ok] = self.terminal.take(
				end_id[not_terminal] - 1, mode = 'wrap').reshape(-1, 1)
			cnt += num_ok
