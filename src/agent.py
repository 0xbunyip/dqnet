...
class Agent:
	...
	FINAL_EXPLORATION_FRAME = 500

	def __init__(self, num_action, frame_height, frame_width, mbsize):
		self.num_action = num_action
		self.mbsize = mbsize
		self.obs_count = 0
		self.eps_decay_rate = (Agent.FINAL_EXPLORATION - Agent.INITIAL_EXPLORATION) * 1.0 / Agent.FINAL_EXPLORATION_FRAME
	...
	def get_latest_action(self):
		...
		if np.random.rand() < eps:
			return np.random.randint(self.num_action)
		action = self.network.get_action(self.exp.get_last_state())
		return action
