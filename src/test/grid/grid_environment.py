...
class Environment:
	...
	def train_agent(self, epoch_count):
		num_test = 100
		before_reward = 0
		# before_reward, _ = self.test(num_test, print_Q = True)
		...
		after_reward, action_list = self.test(10, print_Q = True)
	...
	def test(self, num_episode, max_episode_length = 1000, print_Q = False):	
		total_reward = 0.0
		obs = np.zeros((Environment.FRAME_HEIGHT, Environment.FRAME_WIDTH), dtype = np.uint8)
		is_terminal = False
		action_list = np.zeros((4, 1))
		...
		for episode in range(num_episode):
			...
			while not self.api.game_over() and step_cnt < max_episode_length:
				self._get_screen(obs)
				action_id = self.agent.get_action(obs, e_greedy = False, print_Q = print_Q)
